...
def createLink(row):
    s = '<a href="https://codeforces.com/problemset/' + \
        str(row['contestId']) + "/"+row['index']+">"+row['name']+"</a>"
    return s
...
def user_dataframe(handle):
    ...
    user_df = pd.DataFrame(
        columns=['contestId', 'index', 'name', 'rating', 'tags', 'verdict'])
    for row in response['result']:
        ...
        ver = row['verdict']
        dit = {'contestId': cont_id, 'index': prob_ind,
               'name': prob_name, 'rating': rat, 'tags': tag, 'verdict': ver}
...
def problems_recommended(strong_areas, weak_areas):
    ...
    strong_areas_problem['links'] = strong_areas_problem.agg(
        lambda x: f"<a href=\"https://codeforces.com/problemset/problem/{x['contestId']}/{x['index']}\">{x['name']}</a>", axis=1)
    weak_areas_problem['links'] = weak_areas_problem.agg(
        lambda x: f"<a href=\"https://codeforces.com/problemset/problem/{x['contestId']}/{x['index']}\">{x['name']}</a>", axis=1)
